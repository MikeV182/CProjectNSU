...
struct UserData topUsers[MAX_USERS];
...
void transfer_money(struct data* userFrom) {
    system("cls");
    printf("Enter username to transfer money to: ");
    char username[50];
    scanf("%s", username);
    struct data* userTo = NULL;
    for (int i = 0; i < id_users; ++i) {
...
void insert(struct BTreeNode **root, char *key, long long int money) {
    ...
    struct KeyValue *maxMoneyUser = findMaxMoneyUser(*root);
    if (money > maxMoneyUser->money) {
        strcpy(maxMoneyUser->key, key);
        maxMoneyUser->money = money;
    }
}
